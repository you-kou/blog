...
返回一个路径，该路径是消除了冗余名称元素的此路径。
 此方法的精确定义取决于具体实现，但通常它从此路径派生出一个不包含冗余名称元素的路径。在许多文件系统中，"." 和 ".." 是特殊名称，分别表示当前目录和父目录。在这样的文件系统中，所有出现的 "." 都被视为冗余。如果 ".." 前面有一个非 ".." 的名称，则这两个名称都被视为冗余（识别此类名称的过程会重复进行，直到不再适用为止）。

此方法不会访问文件系统；路径可能无法定位到实际存在的文件。从路径中消除 ".." 和其前面的名称可能会导致路径定位到与原始路径不同的文件。当前面的名称是符号链接时，可能会出现这种情况。

**返回值**
生成的路径，如果此路径不包含冗余名称元素，则返回此路径；如果此路径没有根组件且所有名称元素都是冗余的，则返回一个空路径。
